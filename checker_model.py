...
class CheckerModel:
    ...
    def ia_move(self, model,to_maximise, depth, cost_number):
        if model == "random":
            selected_piece_position, move_position = self.random_model_predict()
            self.move_piece(selected_piece_position, move_position)

        elif model == "minimax":
            best_selected_piece_position, best_move_position = self.minimax_model_predict(to_maximise=to_maximise,depth=depth)
            self.move_piece(best_selected_piece_position, best_move_position)

        elif model == "monte_carlos":
            selected_piece_position, move_position = self.monte_carlo(self,cost_number)
            self.move_piece(selected_piece_position,move_position)

        elif model == "minimax_montecarlo":
            selected_piece_position, move_position = self.minimax_montecarlo(to_maximise,depth,cost_number)
            self.move_piece(selected_piece_position,move_position)

    def minimax_model_predict(self, depth,to_maximise):
        ...
        for possible_action in possible_actions:
            ...
            score = self.minimax(robot_turn=False, to_maximise=to_maximise, depth=depth)

            if to_maximise:
                if best_selected_piece_position is None or score <= best_score:
                    best_score = score
    ...
    def minimax(self, robot_turn,to_maximise,depth, alpha=float('inf'), beta=-float('inf')):
        ...
        if game_state == "draw_game":
            ...
            if depth == 0:
                ...
                for possible_action in possible_actions:
                    self.move_piece(*possible_action)
                    score = self.minimax(robot_turn=not robot_turn, to_maximise=to_maximise, depth=depth-1, alpha=alpha, beta=beta)
                    if to_maximise:
    ...
    def monte_carlo(self,game_state,cost_number):
        ...
        if game_state == "draw_game":
            ...
            possible_move = list(self.dict_of_possible_moves.items())
            for move,_ in possible_move:
                win = 0
                for _ in range(cost_number):
                    model = copy.deepcopy(self)
                    model.move_piece(*move)
                    winner = model.simulation()
                    if winner == 1:
                        win += 1
                win_rate = win / cost_number
    ...
    def minimax_montecarlo(self,to_maximise,depth,cost_number):
        game_state=self.check_game_state()
        #possible_moves = self.dict_of_possible_moves
        best_move= None
        if to_maximise:
            best_score = float("inf")
        else:
        ...
        if game_state == "draw_game":
            ...
            possible_moves = list(self.dict_of_possible_moves.items())

            for move, _ in possible_moves:
                model = copy.deepcopy(self)
                model.move_piece(*move)
                score = model.minimax(self,to_maximise, depth)
                if to_maximise and  score <= best_score:
                    best_score = score
                    best_move = move
            ...
            if best_score == float("-inf"):
                best_move = self.monte_carlo(cost_number)
            if best_move is None:
                # Retournez par exemple le premier mouvement possible
